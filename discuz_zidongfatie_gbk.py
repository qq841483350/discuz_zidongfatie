#coding:utf8
#discuz自动登陆并发帖   注意不要启用登陆验证码,在后台版块编辑和用户-用户组编辑里设置 允许使用 HTML 代码
import urllib2,urllib,cookielib,requests,re,time,xlrd,html2UBB,get_tags,random,sys
reload(sys)
...
def discuz(domain="网站域名注意前面 不带http:// 后面也不能事/",exclename='存放用户名密码的excel表格的名字如：username.xls',fid='栏目ID（栏目URL的forum-后面的第一个值）',subject='帖子标题',message='帖子内容',tags='帖子的关键词'):
    ...
    if """window.location.href""" in html:
        ...
        post_data={
                ...
                "subject":subject.decode('utf8').encode('gbk'),   #帖子标题
                "message":message.decode('utf8').encode('gbk','ignore'),   #帖子内容
                "tags":tags.decode('utf8').encode('gbk'),                    #关键词
                "htmlon":"1",
        ...
        if subject.decode('utf8').encode('gbk') in html:
            print '发表成功标题是：'.decode('utf8'),subject
            #del headers['Cookie']
        elif subject.decode('utf8').encode('gbk') not in html:
            print '发布失败,请检查您的用户名和密码是否正确:'.decode('utf8'),'用户名是：'.decode('utf8'),username
        else:
            pass
    elif '登录失败'.decode('utf8').encode('gbk') in html:
